...
class ProgressBarAction(ItemAction):
  def __init__(self, width=600):
    ...
    self.text_color = t.TEXTO2  # ORBIT MUTED (was rl.GRAY)
    self._font = gui_app.font(FontWeight.NORMAL)

  def update(self, progress, text, show_progress=False, text_color=t.TEXTO2):  # ORBIT MUTED default
  ...
  def _render(self, rect: rl.Rectangle):
    ...
    if self.show_progress:
      ...
      if inner_rect.width > 0:
        ...
        rl.draw_rectangle_rounded(rl.Rectangle(inner_rect.x, inner_rect.y, fill_width, inner_rect.height), 0.2, 10, t.ACCION)  # ORBIT BLUE (relleno de botón) -> acción

...
class SectionHeaderSP(Widget):
  def __init__(self, text: str, seccion: str = 'cabina'):
    super().__init__()
    self._text = text
    self._font = gui_app.font(FontWeight.BOLD)
    self._rect = rl.Rectangle(0, 0, 0, _HEADER_HEIGHT)
    self._color = t.con_alfa(t.SECCION[seccion], 210 / 255)
  ...
  def _render(self, _):
    ...
    rl.draw_text_ex(self._font, self._text, rl.Vector2(self._rect.x + 8, text_y), _FONT_SIZE, _LETTER_SPACING, self._color)
